...
export const paddedPrice = (price: string, symbol: string) => {
  ...
  return cleaned.split(symbol).join(`${symbol} `).trim();
};
...
export const getFormattedCart = (data: IFormattedCartProps) => {
  ...
  givenProducts.forEach((item) => {
    ...
    const convertedCurrency = String(item.total).replace(/[^0-9.-]+/g, '');
    ...
    const product: CartProduct = {
      ...
      totalPrice: cleanPrice(String(item.total)),
  ...
  formattedCart.totalProductsPrice = cleanPrice(String(data.cart.total));
