...
const CartContents = () => {
  ...
  }, [data, syncWithWooCommerce, clearWooCommerceSession]);
      ...
      {data?.cart?.contents?.nodes?.length ? (
        <>
              ...
              >
                ...
                <div className="flex-grow ml-4">
                  ...
                  <p className="text-text-muted">
                    {t('currency_symbol')} {getUnitPrice(item.subtotal, item.quantity)}
                  </p>
                </div>
                <div className="flex items-center">
                  <input
                    ...
                    aria-label={`${t('quantity')} ${item.product.node.name}`}
                  />
                  <Button
                    handleButtonClick={() =>
          ...
          <div className="bg-surface rounded-lg p-6 md:w-full">
            <div className="flex justify-end mb-4">
              <span className="font-semibold pr-2 text-text">{t('subtotal')}:</span>
              <span className="text-text">{cartTotal}</span>
            </div>
            {!isCheckoutPage && (
              <div className="flex justify-center mb-4">
                <Link href="/checkout" passHref>
                  <Button variant="primary" fullWidth>{t('proceed_to_checkout')}</Button>
                </Link>
              </div>
            )}
          </div>
        ...
        <div className="text-center">
          ...
          <Link href="/products" passHref>
            <Button variant="primary">{t('continue_shopping')}</Button>
          </Link>
        </div>
      )}
      {updateCartProcessing && (
        <div className="fixed inset-0 flex items-center justify-center bg-overlay bg-opacity-50">
          <div className="bg-surface p-4 rounded-lg">
            <p className="text-lg mb-2 text-text">{t('updating_cart')}</p>
            <LoadingSpinner />
          </div>
        </div>
      )}
